#!/usr/bin/env python3
"""
Bayes_opt_half_RSI.py  (PARQUET-ONLY)
...
- This script: buy/sell from half_RSI hot_sm crossing 0 (NO trend filter)

Everything else matches Dynamic_SR:
- PARQUET-only loading + ensure_ohlc
- Fill modes: same_close / next_open
- Equity model: multiplicative equity (1.0 start), full-notional per trade
...
- Auto mode: min_tp2sl_eff adapts off atrPeriod (recommended):
...
def backtest_half_rsi_dynamic_engine(
    df: pd.DataFrame,
    *,
    atrPeriod: int,
    slMult: float,
    ...
    # ----- half_RSI signal pipeline (PURE momentum, no trend filter) -----
    sigClose = ha_c
    ...
    for i in range(start + 1, n):
        ...
        # Manage open position (identical to dynamic_SR)
        if in_pos:
            bars_in_trade += 1
...
def evaluate_params_on_files(
    ...
    *,
    ...
    for p in file_paths:
        ...
        st = backtest_half_rsi_dynamic_engine(
            df,
            atrPeriod=atrPeriod,
            slMult=slMultiplier,
            tpMult=tpMultiplier,
...
def parse_args() -> argparse.Namespace:
    ...
    # --- TP/SL asymmetry constraint (dynamic_SR compatible flags; auto adapts off atrPeriod here) ---
    ap.add_argument("--min-tp2sl", type=float, default=1.30,
                    help="Require slMultiplier > min_tp2sl * tpMultiplier (fixed mode).")

    ap.add_argument("--tp2sl-auto", action="store_true",
                    help="If set, enforce an adaptive TP/SL asymmetry constraint based on atrPeriod.")
    ap.add_argument("--tp2sl-base", type=float, default=1.25,
                    help="Base min_tp2sl at atrPeriod = tp2sl-sr0 (tp2sl-sr0 is repurposed as atr0 here).")
    ap.add_argument("--tp2sl-sr0", type=float, default=30.0,
                    help="Reference atrPeriod (atr0) for tp2sl-base (flag name kept for compatibility).")
    ap.add_argument("--tp2sl-k", type=float, default=0.01,
                    help="Slope: how much min_tp2sl increases per +1.0 atrPeriod.")
...
def main() -> None:
    ...
    if args.report_only:
        ...
        for fm in fills:
            best_score_single, overall, per, _, _ = evaluate_params_on_files(
                ...
                slow_window=int(args.slow_window_fixed),
        ...
        print(f"half_RSI: slow_window={args.slow_window_fixed}, shift={args.shift_fixed}, smooth_len={args.smooth_len_fixed}")
        return

    # =========================
    # OPTUNA (robust across both fills like dynamic_SR)
    # =========================
    def objective(trial: optuna.Trial) -> float:
        ...
        # --- TP/SL asymmetry constraint (fixed or auto; auto adapts off atrPeriod) ---
    ...
    print(f"slow_window: {best_params['slow_window']}")
    ...
    # ===== Reproducible command line (Dynamic_SR style) =====
    if args.tp2sl_auto:
        tp2sl_str = (
            f"--tp2sl-auto "
            f"--tp2sl-base {args.tp2sl_base} "
            f"--tp2sl-sr0 {args.tp2sl_sr0} "
            f"--tp2sl-k {args.tp2sl_k} "
            f"--tp2sl-min {args.tp2sl_min} "
            f"--tp2sl-max {args.tp2sl_max} "
        )
    else:
        tp2sl_str = f"--min-tp2sl {args.min_tp2sl} "

    print(
        f"\nDone: Bayes_opt_half_RSI.py "
        ...
        f"{tp2sl_str}"
        f"--atrPeriod-fixed {best_params['atrPeriod']} "
        f"--slMultiplier-fixed {best_params['slMultiplier']} "
        f"--tpMultiplier-fixed {best_params['tpMultiplier']} "
        f"--slow_window-fixed {best_params['slow_window']} "
        f"--shift-fixed {best_params['shift']} "
        f"--smooth_len-fixed {best_params['smooth_len']}"
    )
